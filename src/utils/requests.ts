...
import { ApiGetTaskDetail, ApiGetTasks } from "src/models/Task";


// Authentication
const signIn = async ({email,password}: {email: string, password: string}) =>{
    const response = await useApi<ApiSignIn>('accounts/signin/', 'POST', {email,password}, false);
    return response;
}

const getUser = async() => {
    const response = await useApi<ApiGetUser>('accounts/user/');
    return response;
}
...
const getAnTask = async (id: number) => {
    const response = await useApi<ApiGetTaskDetail>(`companies/tasks/${id}`);
    return response;
}
...
const addTask = async(
    ...
    const response = await useApi<ApiGetTaskDetail>('companies/tasks', 'POST', { title, description, due_date, employee_id, status_id });
    return response;
}
